...
;; See https://stackoverflow.com/questions/9729549/why-do-we-need-funcall-in-lisp
;; for an explanation for why the following don't work.  'add-100' and 'add-500'
;; are both defined in the _variable_ namespace, not _function_ namespace, so
;; are not found by list interpreter.  These are like trying to do:
;; (let ((a 1))
;;   (a 2 3))
;; 'add-100' and 'add-500' are variables just like 'a'
   ;; (assert-equal 103 (add-100 3))
   ;; (assert-equal 503 (add-500 3))))
